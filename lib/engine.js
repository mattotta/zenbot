  ...
  return function (s) {
    ...
    function executeSignal (signal, _cb, size, is_reorder, is_taker) {
      ...
      syncBalance((signal !== 'buy' && signal !== 'sell'), function (err) {
        ...
        getQuote(true, function (err, quote) {
          if (err) {
          ...
          if (signal === 'buy') {
            if (!is_taker) {
              ...
              if (n(quote.ask).subtract(bid).value() >= n(s.product.increment).value()) {
                quote.bid = bid
              }
            }
            ...
            if (!is_taker) {
              ...
              if (n(ask).subtract(quote.bid).value() >= n(s.product.increment).value()) {
                quote.ask = ask
              }
            }
