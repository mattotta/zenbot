  ...
  return function (s) {
    ...
    function executeStop (do_sell_stop) {
      ...
      if (s.my_trades.length) {
        ...
        var last_trade =  {price: s.start_price}
        if (n(s.start_capital).subtract(n(s.balance.currency)).value() > n(s.balance.currency).value()) {
          last_trade.type = 'buy'
        } else {
    ...
    function placeOrder (type, opts, cb) {
      ...
      if (so.mode !== 'live') {
        ...
        if (type === 'buy') {
          pushMessage('Buying ' + s.exchange.name.toUpperCase(), 'placing buy order at ' + fc(order.price) + '\n')
        } else {
          pushMessage('Selling ' + s.exchange.name.toUpperCase(), 'placing sell order at ' + fc(order.price) + '\n')
        }
    ...
    function executeSignal (signal, _cb, size, is_reorder, is_taker) {
      ...
      syncBalance((signal !== 'buy' && signal !== 'sell'), function (err) {
        ...
        getQuote(function (err, quote) {
          ...
          if (signal === 'buy') {
            if (!is_taker) {
              let bid = n(quote.bid).add(s.product.increment).value()
              if (bid < n(quote.ask).value()) {
                quote.bid = bid
              }
            }
            ...
            if (!is_taker) {
              let ask = n(quote.ask).subtract(s.product.increment).value()
              if (ask > n(quote.bid).value()) {
                quote.ask = ask
              }
            }
