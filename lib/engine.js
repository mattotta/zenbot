...
module.exports = function container (get, set, clear) {
  ...
  return function (s) {
    ...
    function executeOrder (trade) {
      ...
      if (s.buy_order) {
        if (so.mode === 'live' || (trade.time > s.buy_order.orig_time && trade.price >= Number(s.buy_order.price))) {
        ...
        if (so.mode === 'live' || (trade.time > s.sell_order.orig_time && trade.price <= Number(s.sell_order.price))) {
          if (trade.done_reason === 'canceled') {
            if (so.stats) {
              var message = '\nsell order canceled\n'
              console.log((message).red)
