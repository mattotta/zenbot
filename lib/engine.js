  ...
  return function (s) {
    ...
    function executeStop (do_sell_stop) {
      if (so.sell_stop_pct && s.sell_stop) {
        ...
        if (buy_stop < s.buy_stop_pct) {
          s.buy_stop_pct = buy_stop
        }
      }
      var stop_signal
    ...
    function executeSignal (signal, _cb, size, is_reorder, is_taker) {
      ...
      syncBalance((signal !== 'buy' && signal !== 'sell'), function (err) {
        ...
        getQuote(function (err, quote) {
          ...
          if (signal === 'buy') {
            if (!is_taker && quote.bid !== quote.ask) {
              quote.bid = n(quote.ask).subtract(0.01).value()
            }
            price = n(quote.bid).subtract(n(quote.bid).multiply(so.markdown_buy_pct / 100)).format(s.product.increment, Math.floor)
            if (!size) {
              total = n(s.balance.currency).multiply(so.buy_pct).divide(100).format('0.00000000')
              if (is_taker) {
            ...
            if (!is_taker && quote.bid !== quote.ask) {
              quote.ask = n(quote.bid).add(0.01).value()
            }
            ...
            if ((s.product.min_size && Number(size) >= Number(s.product.min_size)) || (s.product.min_total && n(size).multiply(price).value() >= Number(s.product.min_total))) {
              ...
              if (so.max_sell_loss_pct && sell_loss > so.max_sell_loss_pct) {
                ...
                if (n(s.balance.asset).subtract(s.balance.asset_hold || 0).value() < n(size).value()) {
                  ...
                  pushMessage('Selling ' + s.exchange.name.toUpperCase(), 'placing sell order at ' + fc(price) + ', ' + fc(Number(price) - quote.ask) + ' over best ask\n')
                  doOrder()
                }
              }
            }
    ...
    function executeOrder (trade) {
      var price, total, size, fee = 0
      if (!so.order_type) {
        so.order_type = 'maker'
      }
      if (s.buy_order) {
        if (so.mode === 'live' || (trade.time > s.buy_order.orig_time && (s.buy_order.order_type === 'taker' || trade.price <= Number(s.buy_order.price)))) {
          if (trade.done_reason === 'canceled') {
            if (so.stats) {
              var message = '\nbuy order canceled\n'
              console.log((message).red)
              if (so.mode === 'live' || so.mode === 'paper') {
                notify.pushMessage('Buy ' + s.exchange.name.toUpperCase(), message)
              }
            }
            delete s.buy_order
            delete s.buy_stop
        ...
        if (so.mode === 'live' || (trade.time > s.sell_order.orig_time && (s.sell_order.order_type === 'taker' || trade.price >= Number(s.sell_order.price)))) {
          if (trade.done_reason === 'canceled') {
            if (so.stats) {
              var message = '\nsell order canceled\n'
              console.log((message).red)
              if (so.mode === 'live' || so.mode === 'paper') {
                notify.pushMessage('Sell ' + s.exchange.name.toUpperCase(), message)
              }
            }
            delete s.buy_order
            delete s.buy_stop
            ...
            if (period_id !== s.period.period_id) {
              s.strategy.onPeriod.call(s.ctx, s, function () {
                ...
                if (!s.in_preroll && !so.manual) {
                  executeStop(true)
                  if (s.signal) {
                    executeSignal(s.signal, null, null, false, (s.acted_on_stop === true))
                  }
                }
                writeReport()
