...
let runCommand = (strategy, cb) => {
  countArr.push(1);
  let command = `./zenbot.sh sim ${simArgs} --strategy=trust_distrust --period_length=${strategy.periodLength}m --sell_threshold=${strategy.sell_threshold} --sell_threshold_max=${strategy.sell_threshold_max} --sell_min=${strategy.sell_min} --buy_threshold=${strategy.buy_threshold} --days=30`;
...
parallel(tasks, PARALLEL_LIMIT, (err, results) => {
  ...
  let csv = json2csv({
    data: results,
    fields: ['roi', 'errorRate', 'wlRatio', 'frequency', 'endBalance', 'wins', 'losses', 'period_length', 'days', 'sellThreshold', 'sellThresholdMax', 'sellMin', 'buyThreshold', 'params'],
