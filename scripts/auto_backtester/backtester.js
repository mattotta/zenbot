...
let runCommand = (strategy, cb) => {
  ...
  let strategyArgs = {
    ...
    trend_ema: `--trend_ema=${strategy.trend_ema} --oversold_rsi=${strategy.oversold_rsi} --oversold_rsi_periods=${strategy.oversold_rsi_periods} --neutral_rate=${strategy.neutral_rate} --neutral_rate_min=${strategy.neutral_rate_min} --reversed=${strategy.reversed}`,
  ...
  let command = `${zenbot_cmd} sim ${simArgs} ${strategyArgs[strategyName]} --period=${strategy.period_length} --min_periods=${strategy.min_periods} --silent --filename=none`;
...
let strategies = {
  cci_srsi: objectProduct({
    period_length: ['1m', '2m', '5m', '10m', '20m', '30m', '1h'],
    min_periods: [52, 200],
    rsi_periods: [14, 20],
    srsi_periods: [14, 20],
    srsi_k: [3, 9],
...
parallel(tasks, PARALLEL_LIMIT, (err, results) => {
  ...
  let filedsGeneral = ['roi', 'vsBuyHold', 'errorRate', 'wlRatio', 'frequency', 'endBalance', 'buyHold', 'wins', 'losses', 'period_length', 'min_periods', 'days'];
  ...
  let fields = {
    ...
    trend_ema: filedsGeneral.concat(['trendEma', 'neutralRate', 'neutralRateMin', 'oversoldRsiPeriods', 'oversoldRsi', 'params'])
  };
  let fieldNames = {
    ...
    trend_ema: filedNamesGeneral.concat(['Trend EMA', 'Neutral Rate', 'Neutral Rate Minimum', 'Oversold RSI Periods', 'Oversold RSI', 'Full Parameters'])
  };
  let csv = json2csv({
    data: results,
